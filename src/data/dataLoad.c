...
int dataLoad(Movie *movie) {
  ...
  while (fgets(line, MAXCHAR, fp) != NULL && counter < 250) {
    // While loop som anvender funktionen fgets. Den skal bruge et sted at
    // opbevare dataen, maks antal characters den skal læse, og fra hvilken fil.
    // Hvis det lykkedes opbevares dataen i det første parameter. Stopper hvis
    // filen er tom, og derfor er NULL.
